...
select course.cname, teacher.tname from course left join teacher on course.teacher_id = teacher.tid;
...
SELECT s.sid, s.sname, COUNT(sc.sid) AS pick_num
...
-- 6、 查询姓李老师的个数
SELECT '李老师个数' AS tname, COUNT(tid)
FROM teacher
WHERE tname LIKE '李%';
...
-- 先查询出李平老师任教的课程
SELECT course.cid, course.cname, teacher.tname
FROM course
	INNER JOIN teacher ON course.teacher_id = teacher.tid
WHERE tname = '李平老师';
...
WHERE t.tname != '李平老师';-- 23、查询所有选修了学号为1的同学选修过的一门或者多门课程的同学学号和姓名；
